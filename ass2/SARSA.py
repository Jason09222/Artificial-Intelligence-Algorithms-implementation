...
class SARSA(Agent):
    def reset(self):
        ...
        with open("random_numbers.txt", "r") as files:
            self.rnd = [float(num.strip()) for num in files.readlines()]
            
    #TODO Use your modified action selection method
    def action_selection(self, S):
        Qs = self.Q[S]
        maxQ = max(Qs)
        minQ = min(Qs)
        Action_Best = list(where(Qs == maxQ)[0])
        Action_Worst = list(where(Qs == minQ)[0])

        if self.rnd[self.record_index] <= self.epsilon:
            self.record_index = self.record_index + 1
            return Action_Worst[0]
        else:
            self.record_index = self.record_index + 1
    ...
    def do_step(self, S, act, logfile=None):
        ...
        #TODO Implement do_step method similarly as Q-learning but using on-policy method SARSA to update state-action pairs.
        S = self.get_S()
        ...
        # Update return
        self.G += R
        A_Sp = self.action_selection(Sp)

       # (Q(S', 'A'))
    
        # Update Q for this state/action pair
        delta = R + self.gamma * self.Q[Sp][A_Sp] - self.Q[S][A]
