...
class IdDfsAgent(Agent):
    ...
    def start_agent(self):
        ...
        for depth in range(1, gridworld.DEFAULT_W * gridworld.DEFAULT_H + 1): # loo from depth 1 to depth gridworld.DEFAULT_W * gridworld.DEFAULT_H
            path = dfs.find_path(self.gw.immtileneighbours, # use bfs method, each loop add 1 to the depth for exploring
                                    self.start,
                                    self.goal,
                                    [],
                                    lambda tile: not self.gw.tileblocked(*self.gw.indextopos(tile)),
                                    depth)
            if path: # if we find the target/goal, jump out of the loop and we get the optimal path
                break
        self.path = [] 
        if path:
            self.path = path # if we find the path, we load the outcome to self.path, if not find the path, self.path remains []
